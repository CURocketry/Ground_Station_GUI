...
public class GSView extends Main {
    public static Font bold16Font= new Font("Arial", Font.BOLD, 16);
    public static Font italic24Font= new Font("Arial", Font.ITALIC, 24);

    public LaunchTab launch;
    public HazardTab hazard;
    public RecoveryTab recovery;
    ...
    public void addGPSNodes(GPSObject[] objects){
    ...
    public GSView(GSModel model, JFrame mainFrame) {
        //Add altitude/latitude/longitude data to arraylist...

        addListener();

        launch= new LaunchTab(model);
        hazard= new HazardTab(model);
        recovery= new RecoveryTab(model,contentPanePrivate);


        JTabbedPane tabbedPane = new JTabbedPane();
        tabbedPane.addTab("Launch", null, launch,
                          "Rocket section and trajectory data");
        tabbedPane.addTab("Hazard Detection", null, hazard,
                          "Hazard Detection Camera data");
        tabbedPane.addTab("Recovery", null, recovery,
     ...
     */
    public static GSView setup(GSModel model, final String[] argArray) {
        I18n.init();
        Main.checkJava6();
        ...
        final Main main= new GSView(model, mainFrame);
...
//JSOM soon will no longer
    ...
    //Steven Dourmashkin's Added functions...
    public static JLabel customJLabel(String text, int position, Font font){
        JLabel label= new JLabel(text, position);
        label.setFont(font);
        return label;
    }
    public static JLabel customJLabel2(String text, int position, Font font, Color c){
        JLabel label= new JLabel(text, position);
        label.setFont(font);
    ...
    public void addController(GSApp controller) {
        /*
        launch.runBtn.addActionListener(controller);
        launch.runBtn.setActionCommand(GSApp.BUTTON_RUN);
        */

        launch.runBtn.addItemListener(controller);
        launch.enablePayload.addItemListener(controller);

        launch.sepTestBtn.addActionListener(controller);
        launch.sepTestBtn.setActionCommand(GSApp.BUTTON_SEPTEST);

        hazard.openHzrdFileBtn.addActionListener(controller);
        hazard.openHzrdFileBtn.setActionCommand(GSApp.BUTTON_OPENHZRD);

        hazard.getHzrdBtn.addActionListener(controller);
        hazard.getHzrdBtn.setActionCommand(GSApp.BUTTON_GETHZRD);

        recovery.selNodesBtn.addActionListener(controller);
        recovery.selNodesBtn.setActionCommand(GSApp.BUTTON_SELNODES);

        recovery.openOsmBtn.addActionListener(controller);
        recovery.openOsmBtn.setActionCommand(GSApp.BUTTON_OPENOSM);
    }
